...
var delBomb_timer; // Таймер для удаления бомбы
var score = 0; // Результат
var bomb_cell; // Ячейка с бомбой

var result = document.getElementById('result');
result.innerHTML = "<p>" + "Ваш результат: " + score + "</p>";

function init() {
    prepareGameField(); // Генерация поля    // Здесь можно сделать замыкание function() {}
    ...
    */
    ...
    document.getElementById('snake-renew').addEventListener('click', refreshGame); // Здесь можно сделать замыкание function() {}
...
function startGame() {
    gameIsRunning = true;
    respawn();//создали змейку   // Здесь можно сделать замыкание function() {}

    snake_timer = setInterval(move, SNAKE_SPEED);//каждые 200мс запускаем функцию move
    setTimeout(createFood, 5000);
    bomb_timer = setInterval(createBomb, 5100); //каждые 5100мс запускаем функцию createBomb
}
...
function createBomb() {
    ...
    while (!bombCreated) { //пока бомбу не создали
        ...
        bomb_cell = document.getElementsByClassName('cell-' + bomb_y + '-' + bomb_x)[0];
        ...
        if (!bomb_cell_classes.includes('snake-unit') && !bomb_cell_classes.includes('food-unit')) {
            ...
            bombCreated = true;
            delBomb_timer = setTimeout(deleteBomb, 4900); // Здесь можно сделать замыкание
        }
    }
}
...
function move() {
    ...
    // Определяем новую точку и проверяем границы поля
    if (direction == 'x-') {
        if (coord_x == 0) {
    ...
    // 3) Змейка не наехала на бомбу
    //console.log(new_unit);
    if (!isSnakeUnit(new_unit) && !new_unit_classes.includes('bomb-unit')) {
        ...
        finishTheGame();  // Здесь можно сделать замыкание function() {}
    }
}
